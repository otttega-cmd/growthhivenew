/* ================================================================
   Growthhive — Customer Activation & UX Optimization Page Script
   customer-activation-ux.js
...
const SECTION_ACCENTS = [
  ...
  new THREE.Color(0x0B0B1F),    // 2 halves
  new THREE.Color(0xD4F53C),    // 3 caps
  new THREE.Color(0x0B0B1F),    // 4 whatsapp
  new THREE.Color(0xD4F53C),    // 5 process
  new THREE.Color(0x0B0B1F),    // 6 ux demo
  new THREE.Color(0xE8A317),    // 7 studio — CR gold
  new THREE.Color(0x0B0B1F),    // 8 engage
  new THREE.Color(0xD4F53C),    // 9 closer
];
...
window.onLoaderComplete = function() {
  ...
  tl.to('#caxEyebrow', { opacity:1, y:0, duration:1.0, ease:'power3.out' }, .4)
    .to('#caxTitle',   { opacity:1, y:0, duration:1.6, ease:'power4.out' }, .65)
    .to('#caxSub',     { opacity:1, y:0, duration:1.2, ease:'power3.out' }, 1.1)
    .to('#caxMeta',    { opacity:1, duration:1.2, ease:'power3.out' }, 1.5)
    .to('#caxPanelWrap',{ opacity:1, x:0, duration:1.6, ease:'power3.out' }, .9);

  // Activation flow steps stagger in
  gsap.to('#caxFlow .cax-flow-step', { opacity:1, duration:.8, ease:'power2.out', stagger:.18, delay:1.4 });
  ...
  // WhatsApp bubbles appear in sequence when section enters
  ScrollTrigger.create({
    trigger: '#caxWaMock', start: 'top 72%', once: true,
    onEnter: function(){
      gsap.to('#caxWaMock .cax-wa-bubble', { opacity:1, y:0, duration:.5, ease:'power2.out', stagger:.45 });
    }
  });

  // Heatmap blobs fade/scale in when UX demo enters
  ScrollTrigger.create({
    trigger: '#caxWire', start: 'top 72%', once: true,
    onEnter: function(){
      gsap.fromTo(['#heat1','#heat2','#heat3'],
        { opacity:0, scale:.6, transformOrigin:'center' },
        { opacity:1, scale:1, duration:1.1, ease:'power2.out', stagger:.25 });
    }
  });

  // Scene accents
  var sects = [
    ['.cax-hero',0],['.cax-belief',1],['.cax-halves',2],
    ['.cax-caps',3],['.cax-whatsapp',4],['.cax-process',5],
    ['.cax-ux',6],['.cax-studio',7],['.cax-engage',8],['.cax-closer',9]
  ];
