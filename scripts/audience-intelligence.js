/* ================================================================
   Growthhive — Audience Intelligence Discipline Page Script
   audience-intelligence.js
...
const SECTION_ACCENTS = [
  ...
  new THREE.Color(0xD4F53C),    // 3 personas
  new THREE.Color(0x0B0B1F),    // 4 process
  new THREE.Color(0xD4F53C),    // 5 culture
...
window.onLoaderComplete = function() {
  ...
  tl.to('#aiEyebrow', { opacity:1, y:0, duration:1.0, ease:'power3.out' }, .4)
    .to('#aiTitle',   { opacity:1, y:0, duration:1.6, ease:'power4.out' }, .65)
    .to('#aiSub',     { opacity:1, y:0, duration:1.2, ease:'power3.out' }, 1.1)
    .to('#aiMeta',    { opacity:1, duration:1.2, ease:'power3.out' }, 1.5)
    .to('#aiMapWrap', { opacity:1, x:0, duration:1.6, ease:'power3.out' }, .9);

  // Cluster map: hulls fade, then dots scatter in, then labels
  gsap.to(['#hull1','#hull2','#hull3'], { opacity:1, duration:1.4, ease:'power2.out', delay:1.4, stagger:.15 });
  gsap.utils.toArray('.ai-cluster-dot').forEach(function(dot){
    gsap.fromTo(dot,
      { opacity:0, scale:0, transformOrigin:'center' },
      { opacity:.95, scale:1, duration:.5, ease:'back.out(2)', delay:1.5 + Math.random()*0.9 });
  });
  gsap.to(['#lbl1','#lbl2','#lbl3'], { opacity:1, duration:.8, ease:'power2.out', delay:2.6, stagger:.12 });
  ...
  // Persona psychographic bars fill when section enters
  ScrollTrigger.create({
    trigger: '.ai-persona-grid', start: 'top 75%', once: true,
    onEnter: function(){
      gsap.utils.toArray('.ai-psycho-fill').forEach(function(fill, i){
        var w = parseFloat(fill.getAttribute('data-w')) || 0;
        gsap.to(fill, { width: w + '%', duration: 1.2, ease: 'power3.out', delay: 0.2 + (i % 3) * 0.1 });
      });
    }
  });
  ...
  // Scene accents
  var sects = [
    ['.ai-hero',0],['.ai-belief',1],['.ai-caps',2],
    ['.ai-personas',3],['.ai-process',4],['.ai-culture',5],
    ['.ai-engage',6],['.ai-closer',7]
  ];
