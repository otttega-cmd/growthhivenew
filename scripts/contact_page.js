/* ================================================================
   Growthhive — Contact / Start a Project Script
   contact_page.js
...
const SECTION_ACCENTS = [
  new THREE.Color(0xD4F53C),    // 0 hero — chartreuse (the action colour)
  new THREE.Color(0xD4F53C),    // 1 qualifiers — chartreuse
  new THREE.Color(0x0B0B1F),    // 2 proof — navy (grounded)
];
...
window.onLoaderComplete = function() {
  ...
  tl.to('#ctKicker',   { opacity:1, y:0, duration:1.0, ease:'power3.out' }, .3)
    .to('#ctHeadline', { opacity:1, y:0, duration:1.4, ease:'power4.out' }, .55)
    .to('#ctSub',      { opacity:1, y:0, duration:1.2, ease:'power3.out' }, .9);

  // ── Expect items ────────────────────────────────────────────
  ['#ctE1','#ctE2','#ctE3'].forEach((id, i) => {
    gsap.to(id, { opacity:1, x:0, duration:1.2, ease:'power3.out',
      delay: 1.0 + i * 0.2 });
  });

  // ── Work tags ───────────────────────────────────────────────
  for (let i = 1; i <= 10; i++) {
    gsap.to('#ctW'+i, { opacity:1, y:0, duration:.8, ease:'power2.out',
      delay: 1.2 + (i-1) * 0.06 });
  }

  // ── Email block ─────────────────────────────────────────────
  gsap.to('#ctEmail', { opacity:1, duration:1.2, ease:'power3.out', delay:1.6 });

  // ── Calendly wrap ───────────────────────────────────────────
  // Fade in the embed wrapper; hide loader once Calendly fires its event
  gsap.to('#ctCalendlyWrap', { opacity:1, y:0, duration:1.2, ease:'power3.out', delay:.8 });

  window.addEventListener('message', (e) => {
    if (e.data && e.data.event && e.data.event.indexOf('calendly') === 0) {
      const loading = document.getElementById('ctEmbedLoading');
      if (loading) { loading.style.opacity = '0'; loading.style.pointerEvents = 'none'; }
    }
  });

  // Also hide loader after a timeout in case the message doesn't fire
  setTimeout(() => {
    const loading = document.getElementById('ctEmbedLoading');
    if (loading) { loading.style.opacity = '0'; }
  }, 3000);

  // ── Scroll reveals — case refs ──────────────────────────────
  ['#ctC1','#ctC2','#ctC3'].forEach((id, i) => {
    const el = document.querySelector(id);
    if (!el) return;
    ScrollTrigger.create({
      trigger: el, start: 'top 88%', once: true,
      onEnter: () => gsap.to(el, { opacity:1, y:0, duration:1.0,
        ease:'power3.out', delay: i * 0.12 }),
    });
  });

  // ── Scene accent on scroll ──────────────────────────────────
  [
    ['.ct-hero', 0],
    ['.ct-main', 1],
    ['.ct-proof', 2],
