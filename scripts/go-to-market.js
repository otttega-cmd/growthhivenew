/* ================================================================
   Growthhive — Go-to-Market Strategy Discipline Page Script
   go-to-market.js
...
const SECTION_ACCENTS = [
  new THREE.Color(0xD4F53C),    // 0 hero
  new THREE.Color(0xD4F53C),    // 1 belief
  new THREE.Color(0x0B0B1F),    // 2 caps
  new THREE.Color(0xD4F53C),    // 3 motion
  new THREE.Color(0x0B0B1F),    // 4 process
  new THREE.Color(0x009B8A),    // 5 studio — Tender teal
  new THREE.Color(0x0B0B1F),    // 6 engage
  new THREE.Color(0xD4F53C),    // 7 closer
];
...
window.onLoaderComplete = function() {
  ...
  tl.to('#gtmEyebrow', { opacity:1, y:0, duration:1.0, ease:'power3.out' }, .4)
    .to('#gtmTitle',   { opacity:1, y:0, duration:1.6, ease:'power4.out' }, .65)
    .to('#gtmSub',     { opacity:1, y:0, duration:1.2, ease:'power3.out' }, 1.1)
    .to('#gtmMeta',    { opacity:1, duration:1.2, ease:'power3.out' }, 1.5);

  // Hero trajectory — draw the line, then pop nodes + labels in sequence
  var line = document.getElementById('gtmTrajLine');
  var glow = document.getElementById('gtmTrajGlow');

  // Responsive fit: full-bleed slice on desktop, contained meet on mobile
  // so the whole path stays visible on narrow screens.
  var trajSvg = document.getElementById('gtmTraj');
  var mq = window.matchMedia('(max-width: 760px)');
  function fitTraj(e){
    if (!trajSvg) return;
    trajSvg.setAttribute('preserveAspectRatio', e.matches ? 'xMidYMid meet' : 'xMidYMid slice');
  }
  fitTraj(mq);
  if (mq.addEventListener) { mq.addEventListener('change', fitTraj); }
  else if (mq.addListener) { mq.addListener(fitTraj); }

  if (line) {
    gsap.to([line, glow], { strokeDashoffset: 0, duration: 2.6, ease: 'power2.inOut', delay: .8 });
  }
  var nodes  = document.querySelectorAll('.gtm-traj-node');
  var cores  = document.querySelectorAll('.gtm-traj-node-core');
  var labels = document.querySelectorAll('.gtm-traj-label');
  gsap.to(nodes,  { opacity: .9, duration: .5, stagger: .5, delay: 1.6, ease: 'power2.out' });
  gsap.to(cores,  { opacity: 1,  duration: .5, stagger: .5, delay: 1.7, ease: 'power2.out' });
  gsap.to(labels, { opacity: 1,  duration: .5, stagger: .5, delay: 1.9, ease: 'power2.out' });
  ...
  // Phase dots stagger when the motion section enters
  ScrollTrigger.create({
    trigger: '.gtm-phases', start: 'top 75%', once: true,
    onEnter: function(){
      gsap.from('.gtm-phase', { opacity:0, y:30, duration:1, ease:'power3.out', stagger:.18 });
    }
  });

  // Scene accents
  var sects = [
    ['.gtm-hero',0],['.gtm-belief',1],['.gtm-caps',2],
    ['.gtm-motion',3],['.gtm-process',4],
    ['.gtm-studio',5],['.gtm-engage',6],['.gtm-closer',7]
  ];
