/* ================================================================
   Growthhive — Eyowo Case Study Script
   eyowo.js
...
const SECTION_ACCENTS = [
  new THREE.Color(0x1A2744),    // 0 hero — Adire Indigo
  new THREE.Color(0xC8A45A),    // 1 etymology — Old Gold
  new THREE.Color(0x2D4A8C),    // 2 P0 teaser — Cobalt
  new THREE.Color(0xC8A45A),    // 3 P1 ẹyọ — Gold (beginning)
  new THREE.Color(0xE8841A),    // 4 P2 meji — Orange (trust)
  new THREE.Color(0xC8A45A),    // 5 P3 márùn — Gold (growth)
  new THREE.Color(0xE8841A),    // 6 P4 mẹwaa — Orange (people — peak)
  new THREE.Color(0x2D4A8C),    // 7 culture — Cobalt
  new THREE.Color(0xE8841A),    // 8 reveal — Orange (brand moment)
  new THREE.Color(0x1A2744),    // 9 campaign — Indigo
  new THREE.Color(0xC8A45A),    // 10 results — Gold
  new THREE.Color(0xE8841A),    // 11 closer — Orange
];
...
window.onLoaderComplete = function() {
  ...
  // ── Hero reveals — the question arrives slowly ─────────────
  gsap.to('#eyHeroCowrie', { opacity:1, scale:1, duration:2.2, ease:'power3.out', delay:.4 });
  gsap.to('#eyHeroQ',      { opacity:1, y:0, duration:1.8, ease:'power4.out', delay:1.2 });
  gsap.to('#eyHeroSub',    { opacity:1, duration:1.4, ease:'power3.out', delay:2.0 });
  gsap.to('#eyScrollHint', { opacity:1, duration:1.2, delay:2.8 });
  ...
  gsap.utils.toArray('.reveal-u').forEach(el => {
    ScrollTrigger.create({
      ...
      onEnter: () => gsap.to(el, { opacity:1, y:0, duration:1.6, ease:'power3.out' }),
    });
  });
  gsap.utils.toArray('.reveal-s').forEach(el => {
    ScrollTrigger.create({
      trigger: el, start: 'top 82%', once: true,
      onEnter: () => gsap.to(el, { opacity:1, scale:1, duration:1.6, ease:'power3.out' }),
    });
  });

  // ── Counting section — cowrie images reveal ────────────────
  document.querySelectorAll('.ey-count-img img').forEach(img => {
    ScrollTrigger.create({
      trigger: img.closest('.ey-count'), start: 'top 70%', once: true,
      onEnter: () => gsap.to(img, { opacity:1, scale:1, duration:1.8, ease:'power3.out' }),
    });
  });

  // ── Cultural photos ────────────────────────────────────────
  ['#eyCult1', '#eyCult2'].forEach((sel, i) => {
    const cell = document.querySelector(sel);
    if (!cell) return;
    const img = cell.querySelector('img');
    ScrollTrigger.create({
      trigger: cell, start: 'top 78%', once: true,
      onEnter: () => {
        setTimeout(() => {
  ...
  // ── Brand reveal — Eyowo emerges after "Ẹyọ l'ẹyan." ──────
  const brand = document.getElementById('eyBrand');
  if (brand) {
    ScrollTrigger.create({
      trigger: '#eyReveal', start: 'top 55%', once: true,
      onEnter: () => {
        setTimeout(() => {
          gsap.to(brand, { opacity:1, y:0, duration:1.8, ease:'power3.out' });
        }, 800);
      },
    });
  }
  ...
  const sections = [
    ['.ey-hero',0],['.ey-etymology',1],
    ['section.ey-section:not(.ey-etymology):not(.ey-count):not(.ey-culture)',2],
    ['.ey-count:nth-of-type(1)',3],['.ey-count:nth-of-type(2)',4],
    ['.ey-count:nth-of-type(3)',5],['.ey-count.peak',6],
    ['.ey-culture',7],['.ey-reveal',8],
    ['.ey-campaign',9],['.ey-results',10],['.ey-closer',11],
  ];
