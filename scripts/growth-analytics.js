/* ================================================================
   Growthhive — Growth Analytics Discipline Page Script
   growth-analytics.js
...
const SECTION_ACCENTS = [
  ...
  new THREE.Color(0xD4F53C),    // 3 loop
  new THREE.Color(0x0B0B1F),    // 4 cohort
  new THREE.Color(0xD4F53C),    // 5 process
...
window.onLoaderComplete = function() {
  ...
  tl.to('#gaEyebrow',  { opacity:1, y:0, duration:1.0, ease:'power3.out' }, .4)
    .to('#gaTitle',    { opacity:1, y:0, duration:1.6, ease:'power4.out' }, .65)
    .to('#gaSub',      { opacity:1, y:0, duration:1.2, ease:'power3.out' }, 1.1)
    .to('#gaMeta',     { opacity:1, duration:1.2, ease:'power3.out' }, 1.5)
    .to('#gaPanelWrap',{ opacity:1, x:0, duration:1.6, ease:'power3.out' }, .9);

  // Hero bar chart — grow bars from 0
  gsap.utils.toArray('#gaChart .ga-bar').forEach(function(bar, i){
    var h = parseFloat(bar.getAttribute('data-h')) || 0;
    gsap.to(bar, { height: h + '%', duration: 1.2, ease: 'power3.out', delay: 1.3 + i*0.08 });
  });

  // Hero attribution fills + count-up values
  gsap.utils.toArray('.ga-attr-fill').forEach(function(fill, i){
    var w = parseFloat(fill.getAttribute('data-w')) || 0;
    gsap.to(fill, { width: w + '%', duration: 1.4, ease: 'power3.out', delay: 1.6 + i*0.12 });
  });
  gsap.utils.toArray('.ga-attr-val').forEach(function(el, i){
    var v = parseFloat(el.getAttribute('data-v')) || 0;
    var obj = { n: 0 };
    gsap.to(obj, { n: v, duration: 1.4, ease: 'power3.out', delay: 1.6 + i*0.12,
  ...
  ScrollTrigger.create({
    trigger: '.ga-loop-track', start: 'top 75%', once: true,
    onEnter: function(){ gsap.from('.ga-loop-node', { opacity:0, y:30, duration:1, ease:'power3.out', stagger:.18 }); }
  });

  // Cohort heatmap cells reveal in diagonal sweep
  ScrollTrigger.create({
    trigger: '#gaHeatmap', start: 'top 78%', once: true,
    onEnter: function(){
      gsap.utils.toArray('#gaHeatmap .ga-hm-cell').forEach(function(cell){
        var d = parseFloat(cell.getAttribute('data-d')) || 0;
        gsap.to(cell, { opacity: 1, duration: .5, ease: 'power2.out', delay: d * 0.08 });
      });
    }
  });
  ...
  // Scene accents
  var sects = [
    ['.ga-hero',0],['.ga-belief',1],['.ga-caps',2],
    ['.ga-loop',3],['.ga-cohort',4],['.ga-process',5],
    ['.ga-engage',6],['.ga-closer',7]
  ];
