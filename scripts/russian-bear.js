/* ================================================================
   Growthhive — Russian Bear Case Study Script
   russian-bear.js
...
const SECTION_ACCENTS = [
  new THREE.Color(0xC8A668),    // hero — cairngorm gold
  new THREE.Color(0x4A4E58),    // provocation — granite
  new THREE.Color(0xC8A668),    // process — gold
  new THREE.Color(0x5E7E96),    // gems — cold blue
  new THREE.Color(0xC8A668),    // vessel — gold
  new THREE.Color(0x4A4E58),    // launch/result/closer — granite
];
...
window.onLoaderComplete = function() {
  ...
  // ── Hero reveals — slow, reverent ─────────────────────────
  gsap.to('#rbKicker',     { opacity:1, y:0, duration:1.4, ease:'power3.out', delay:.3 });
  gsap.to('.rb-hero-line .w', { opacity:1, y:0, duration:1.6, ease:'power4.out', stagger:.18, delay:.6 });
  gsap.to('#rbHeroSub',    { opacity:1, y:0, duration:1.4, ease:'power3.out', delay:1.4 });
  gsap.to('#rbScrollHint', { opacity:1, duration:1.4, delay:1.9 });
  ...
  gsap.utils.toArray('.reveal-slow').forEach((el) => {
    ScrollTrigger.create({
  ...
  // ── Scene accent shifts ───────────────────────────────────
  const sections = [
    ['.rb-hero',0],['.rb-provocation',1],['.rb-process',2],
    ['.rb-gems',3],['.rb-vessel',4],['.rb-explorations',4],
    ['.rb-mark',2],['.rb-launch',5],['.rb-result',5],
    ['.rb-download',5],['.rb-closer',5],
  ];
  ...
  // ════════════════════════════════════════════════════════
  // ANIMATED STROKE-DRAW — the line traces itself on scroll-in
  // ════════════════════════════════════════════════════════
  function setupDraw(svgId) {
    const svg = document.getElementById(svgId);
    if (!svg) return;
    const strokes = svg.querySelectorAll('path, line, circle, ellipse, polyline');
    strokes.forEach((el) => {
      let len = 0;
      try { len = el.getTotalLength ? el.getTotalLength() : 0; } catch (e) { len = 0; }
      if (!len) {
        // circles/ellipses: approximate via bbox if getTotalLength unsupported
        const b = el.getBBox ? el.getBBox() : null;
        len = b ? 2 * Math.PI * Math.max(b.width, b.height) / 2 : 200;
      }
  ...
  // ── Vessel + mark image reveals ───────────────────────────
  const bottle = document.getElementById('rbBottle');
  if (bottle) {
    ScrollTrigger.create({
      trigger: '.rb-vessel', start: 'top 70%', once: true,
      onEnter: () => gsap.to(bottle, { opacity:1, scale:1, duration:1.8, ease:'power3.out' }),
    });
  }
  const mark = document.getElementById('rbMark');
  if (mark) {
    ScrollTrigger.create({
      trigger: '.rb-mark', start: 'top 72%', once: true,
      onEnter: () => gsap.to(mark, { opacity:1, y:0, duration:1.6, ease:'power3.out' }),
    });
  }

  // ════════════════════════════════════════════════════════
  // THE TWELVE — build 12 dots, light them one by one
  // ════════════════════════════════════════════════════════
  (function twelve() {
    const wrap = document.getElementById('rbTwelve');
    if (!wrap) return;
    const dots = [];
    for (let i = 0; i < 12; i++) {
      const d = document.createElement('div');
      d.className = 'rb-twelve-dot';
      wrap.appendChild(d);
      dots.push(d);
    }
    ScrollTrigger.create({
      ...
      onEnter: () => {
        dots.forEach((d, i) => {
          setTimeout(() => d.classList.add('lit'), 200 + i * 180);
        });
      },
    });
  })();
