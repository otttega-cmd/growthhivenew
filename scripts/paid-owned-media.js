/* ================================================================
   Growthhive — Paid & Owned Media Discipline Page Script
   paid-owned-media.js
...
const SECTION_ACCENTS = [
  ...
  new THREE.Color(0x0B0B1F),    // 1 belief
  new THREE.Color(0xD4F53C),    // 2 channels
  new THREE.Color(0x0B0B1F),    // 3 process
  new THREE.Color(0xD4F53C),    // 4 approaches
  new THREE.Color(0x0B0B1F),    // 5 data-first
  new THREE.Color(0xD4F53C),    // 6 OOH
  new THREE.Color(0x0B0B1F),    // 7 engage
  new THREE.Color(0xD4F53C),    // 8 closer
];
...
window.onLoaderComplete = function() {
  ...
  tl.to('#pomEyebrow',  { opacity:1, y:0, duration:1.0, ease:'power3.out' }, .4)
    .to('#pomTitle',    { opacity:1, y:0, duration:1.6, ease:'power4.out' }, .65)
    .to('#pomSub',      { opacity:1, y:0, duration:1.2, ease:'power3.out' }, 1.1)
    .to('#pomMeta',     { opacity:1, duration:1.2, ease:'power3.out' }, 1.5)
    .to('#pomDashWrap', { opacity:1, x:0, duration:1.6, ease:'power3.out' }, .9);

  // ── Dashboard metric counters ─────────────────────────────────
  const counters = [
    { id:'mCTR',  target: 3.2,  suffix:'%',  dp:1,  delay:1.2 },
    { id:'mROAS', target: 4.1,  suffix:'×',  dp:1,  delay:1.35 },
    { id:'mCPM',  target: 18.4, prefix:'₦',  dp:0,  delay:1.5 },
    { id:'mCPC',  target: 0.22, prefix:'£',  dp:2,  delay:1.65 },
  ];
  counters.forEach(({id, target, suffix='', prefix='', dp, delay}) => {
    const el = document.getElementById(id);
    if (!el) return;
    const obj = { val: 0 };
    gsap.to(obj, {
      val: target, duration: 1.8, ease: 'power2.out', delay,
      onUpdate: () => {
        el.textContent = prefix + obj.val.toFixed(dp) + suffix;
      },
    });
  });

  // ── Sparkline draw-in ─────────────────────────────────────────
  const spark = document.getElementById('pomSparkLine');
  if (spark) {
    const len = spark.getTotalLength ? spark.getTotalLength() : 600;
    gsap.to(spark, {
      strokeDashoffset: 0, duration: 2.2, ease: 'power3.inOut', delay: 1.4,
    });
  }
  ...
  // ── Scene accents ──────────────────────────────────────────────
  const sects = [
    ['.pom-hero',0],['.pom-belief',1],['.pom-channels',2],
    ['.pom-process',3],['.pom-approaches',4],
    ['.pom-data-first',5],['.pom-ooh',6],
    ['.pom-engage',7],['.pom-closer',8],
  ];
